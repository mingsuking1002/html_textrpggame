...
#   - 우선순위 레퍼런스:
#     1) `파친코 히어로 파이어베이스 기반 웹 RPG 프레임워크 설계서.md`
#     2) 플로우차트/와이어프레임 이미지(게임 루프/전투 루프/스토리 루프)
...
#
# TD 설계 원칙:
#   - 스토리는 “노드 기반 데이터(예: storyNodes)”로 설계합니다.
#   - 선택지(choices)는 조건(condition)과 결과(effect)를 갖습니다.
#   - 랜덤 인카운트는 “풀 + 가중치 + 조건” 기반으로 뽑습니다.
#   - 엔딩 진입 조건은 노드/상태 조합으로 판정(코드 하드코딩 금지).

# ────────────────────────────────────────────────────────────
# 5. Firestore 데이터 주도 구조 (필수)
# ────────────────────────────────────────────────────────────
# 5-1. GameData (읽기 전용, 실행 시 1회 로드/캐싱)
#   - `GameData/config`: 전역 설정(시작 HP, 가방 용량, 리롤 비용 등)
#   - `GameData/classes`: 직업/캐릭터 정의(아이콘 경로 포함)
#   - `GameData/symbols`: 기물/아이템 정의(타입, 값, 희귀도, 아이콘)
#   - 확장 시(권장): `GameData/monsters`, `GameData/encounters`, `GameData/story`
#
...
# ────────────────────────────────────────────────────────────
# 6. 프론트엔드 모듈 구조(권장) — 설계서 준수
# ────────────────────────────────────────────────────────────
# `public/js/` 아래 모듈 분할을 기준선으로 둡니다.
#   - `firebase-init.js` : Firebase 초기화 + 로그인/로그아웃
#   - `db-manager.js`    : GameData 로드 + Users 세이브/로드 + 랭킹 기록
#   - `game-state.js`    : state 관리(단일 store) + 구독/발행
#   - `combat-engine.js` : 룰렛/시너지/데미지/승패 판정(순수 로직)
#   - `ui-renderer.js`   : DOM 렌더(이미지 <img src="..."> 기반) + 모달
#   - `app.js`           : 메인 컨트롤러(상태 전이/이벤트 바인딩)
#
